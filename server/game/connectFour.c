    ...
    int check_win_colum(Match * match, int last_move_colum, int last_row, int num_player){
        int count = 0;
        for (int i = 0; i < 4; i++) {
            int row = last_row + i; 
            if (row < rows)
                if (match->grid[row][last_move_colum] == num_player)
                    count++;
    ...
    int check_win_main_diagonal(Match * match, int last_move_colum, int last_row, int num_player){
        int count, cases = 0;
        ...
        while (cases < 4){
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if ((row >= 0) && (row < rows) && (col >= 0) && (col < colums)) 
                    if (match->grid[row][col] == num_player) count++;
    ...
    int check_win_sec_diagonal(Match * match, int last_move_colum, int last_row, int num_player){
